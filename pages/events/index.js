...
export default function PostsListPage({ postsData, tagsListData }) {
  ...
  useEffect(() => {
    ...
    setIsFirstPage(true);
    setIsLastPage(false);
    const selectedTag = tagsListData.find((data) => data.tag === fixedTag);
    if (!selectedTag) {
      return;
    }
    const tagId = tagsListData.find((data) => data.tag === fixedTag)._id;
  ...
  return (
    <>
      ...
      <PostsListPageContainer>
        <PostsListWrapper>
          <GridMax>
            <StyledSectionTitle>{fixedTag}</StyledSectionTitle>
          </GridMax>
          {width > 850 ? (
            <GridMax>
              <DynamicCol ratio={8}>
